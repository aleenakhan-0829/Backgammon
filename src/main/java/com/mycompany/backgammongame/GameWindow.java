...
// FIX #1 – removed the empty outer `Gamewindow` wrapper class.
//           GameWindow is now a proper top-level class in GameWindow.java.
//           Previously it was a non-static inner class, requiring the
//           impossible `new Gamewindow().new GameWindow(net)` to instantiate.

/**
 * GameWindow - The main game window.
 * Contains the board panel, dice/roll controls, a chat area,
 * a status bar, and handles all incoming server messages.
 */
public class GameWindow extends JFrame {
    ...
    public GameWindow(NetworkClient net) {
        ...
        // Register message handler — runs on network thread, dispatched to EDT
        net.setMessageListener(msg -> SwingUtilities.invokeLater(() -> handleMessage(msg)));

        // FIX #5 – removed the misleading "Tell server we are ready" comment.
        //           READY is intentionally sent inside the PLAYER_NUM handler,
        //           after the server assigns our player number.
    ...
    private JPanel buildRightPanel() {
        ...
        // FIX #2 – was GridLayout(3, 1) but only 2 buttons were ever added,
        //           leaving an empty third row that wasted space and shifted layout.
    ...
    private void handleMessage(String message) {
        ...
        // FIX #4 – all Integer.parseInt calls are now inside try/catch so a
        //           single malformed server message cannot freeze or crash the EDT.
        try {
            switch (cmd) {
                case "PLAYER_NUM":
                ...
                case "START":
                    ...
                    // FIX #3 – reset turn-state flags so a replayed game
                    //           never inherits stale interactive state from the last game.
                    waitingToRoll = false;
                    waitingToMove = false;
                    rollButton.setEnabled(false);
            ...
            // FIX #4 – log and ignore; never let a bad server message crash the UI
            System.err.println("GameWindow: failed to parse message [" + message + "]: " + e.getMessage());
        }
    }
